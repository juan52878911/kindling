...
func TestToolsOfYHealthOf(t *testing.T) {
	...
	// Solo campos de v0.4: se leen igual.
	viejo := &api.Snapshot{Tools: []ToolSpec{{Name: "a"}}, ToolsAt: &at, Health: Unhealthy, HealthErr: "x"}
	if tools, when := ToolsOf(viejo); len(tools) != 1 || when == nil {
		t.Fatalf("ToolsOf sobre campos de v0.4: %v %v", tools, when)
	}
	if h := HealthOf(viejo); h.Status != Unhealthy || h.Error != "x" {
		t.Fatalf("HealthOf sobre campos de v0.4: %+v", h)
	}
	// La anotación manda sobre el campo.
	b, _ := json.Marshal(Tools{Tools: []ToolSpec{{Name: "b"}, {Name: "c"}}})
	nuevo := &api.Snapshot{Tools: []ToolSpec{{Name: "a"}}, Annotations: map[string]json.RawMessage{ToolsKey: b}}
	if tools, _ := ToolsOf(nuevo); len(tools) != 2 {
		t.Fatalf("la anotación debe mandar: %v", tools)
	}
...
func TestSetToolsYHealthContraDaemonViejo(t *testing.T) {
	...
	if err := SetHealth(ctx, c, "eco", true, ""); err != nil {
		t.Fatal(err)
	}
	if strings.Join(f.legacy, ",") != "catalog,health" {
		t.Fatalf("contra un daemon v0.4 debe caer a las rutas antiguas: %v", f.legacy)
	}
}
...
func TestLinksContraDaemonNuevoYViejo(t *testing.T) {
	...
	if ls, err := Links(ctx, c); err != nil || len(ls) != 1 {
		t.Fatalf("links contra daemon v0.4: %v %v", ls, err)
	}
	if strings.Join(f.legacy, ",") != "setlink,links" {
		t.Fatalf("contra un daemon v0.4 debe usar /links: %v", f.legacy)
	}
}
