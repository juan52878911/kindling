...
//
// EL INVITADO NO ES DE FIAR. kling-jev corre dentro de una microVM que sirvió
// una imagen que alguien construyó (`kling jev deploy`); nada impide que esa
...
// (maxJEVGuestAnswerBytes); el CONTENIDO pasaba tal cual a gateway.go, que
...
// modelo (jevDeployLookup, más abajo) antes de que nada de esto llegue a
...
// réplica. En proceso, PredictFull solo pide un puñado (jevDecide pide 5); un
// invitado hostil no debería poder colar miles de entradas minúsculas dentro
// de los 256 KiB que ya acota maxJEVGuestAnswerBytes.
const maxGuestEvidence = 64

// JEVDeployAnnotation es la anotación (pkg/api/annotations.go) que
// `kling jev deploy` graba en el dorado de una tarea backend microvm: las
// etiquetas del .jev horneado dentro y su sha256. Es la fuente de verdad de
// esas etiquetas para el gateway, que no tiene el .jev de origen a mano (puede
...
const JEVDeployAnnotation = "jev.deploy"

// JEVDeployRecord es el valor de JEVDeployAnnotation.
type JEVDeployRecord struct {
	Labels []string `json:"labels"`
	Sha256 string   `json:"sha256"`
}

// jevDeployLookup consulta JEVDeployAnnotation de un dorado. La de verdad
// (clientDeployLookup) pregunta al daemon; los tests ponen una de mentira.
type jevDeployLookup interface {
	jevLabels(ctx context.Context, snapshot string) (JEVDeployRecord, error)
}

// clientDeployLookup es jevDeployLookup sobre el daemon de verdad.
type clientDeployLookup struct{ c *api.Client }

func (d clientDeployLookup) jevLabels(ctx context.Context, snapshot string) (JEVDeployRecord, error) {
	if d.c == nil {
		return JEVDeployRecord{}, errors.New("no daemon client configured")
	}
	snap, err := d.c.Snapshot(ctx, snapshot)
	if err != nil {
		return JEVDeployRecord{}, err
	}
	var rec JEVDeployRecord
	ok, err := snap.Annotation(JEVDeployAnnotation, &rec)
	if err != nil {
		return JEVDeployRecord{}, fmt.Errorf("decoding %s annotation: %w", JEVDeployAnnotation, err)
	}
	if !ok || len(rec.Labels) == 0 {
		return JEVDeployRecord{}, fmt.Errorf(
			"snapshot %q has no %s annotation with labels (deployed before this gateway could pin them; redeploy with kling jev deploy)",
			snapshot, JEVDeployAnnotation)
	}
	return rec, nil
}
...
// (`kling jev deploy -replace`), así que no hace falta preguntar en cada
// clasificación; este plazo acota cuánto tarda el gateway en enterarse de uno.
const deployCacheTTL = 30 * time.Second

// deployLabels da las etiquetas válidas de un modelo jev backend microvm,
...
func (g *Gateway) deployLabels(ctx context.Context, snapshot string) ([]string, error) {
	...
	rec, err := g.deploy.jevLabels(ctx, snapshot)
	if err != nil {
...
// (JEVDeployAnnotation): sin él, el gateway no tiene con qué validar lo que
...
// toda probabilidad que JEV calcula, calibrada o no.
...
func validateGuestReply(labels []string, gr jevGuestResponse) (jev.Prediction, error) {
	if len(labels) == 0 {
		return jev.Prediction{}, errors.New("no label set to validate against")
	}
	if len(labels) > jev.MaxLabels {
		return jev.Prediction{}, fmt.Errorf("deploy record has %d labels, more than %d", len(labels), jev.MaxLabels)
	}
	...
	if gr.Label == "" || !allowed[gr.Label] {
		return jev.Prediction{}, fmt.Errorf("unknown label %q", truncUTF8(gr.Label, 128))
	}
	if !finite01(gr.Prob) {
		return jev.Prediction{}, fmt.Errorf("prob %v is not finite in [0,1]", gr.Prob)
	}
	// El umbral de una clase puede llegar a jev.NeverConfident (2.0, "nunca
	// confiado": ver pkg/jev.Model.Init y TaskConfig.Thresholds en config.go,
	...
	if !finiteFloat(gr.Threshold) || gr.Threshold < 0 || gr.Threshold > jev.NeverConfident {
		return jev.Prediction{}, fmt.Errorf("threshold %v is not finite in [0,%v]", gr.Threshold, jev.NeverConfident)
	}
	switch gr.Decision {
	case "", jev.DecisionConfident, jev.DecisionEscalate:
	default:
		return jev.Prediction{}, fmt.Errorf("unknown decision %q", truncUTF8(gr.Decision, 64))
	}

	if len(gr.Candidates) > len(labels) {
		return jev.Prediction{}, fmt.Errorf("%d candidates for a %d-label model", len(gr.Candidates), len(labels))
	}
	seen := make(map[string]bool, len(gr.Candidates))
	for _, c := range gr.Candidates {
		if c.Label == "" || !allowed[c.Label] {
			return jev.Prediction{}, fmt.Errorf("candidate with unknown label %q", truncUTF8(c.Label, 128))
		}
		if seen[c.Label] {
			return jev.Prediction{}, fmt.Errorf("duplicate candidate label %q", c.Label)
		}
		seen[c.Label] = true
		if !finite01(c.Prob) {
			return jev.Prediction{}, fmt.Errorf("candidate %q: prob %v is not finite in [0,1]", c.Label, c.Prob)
		}
	}

	if len(gr.Evidence) > maxGuestEvidence {
		return jev.Prediction{}, fmt.Errorf("%d evidence entries, more than %d", len(gr.Evidence), maxGuestEvidence)
	}
	for _, e := range gr.Evidence {
		if e.Feature == "" || len(e.Feature) > jev.MaxLabelBytes || !finiteFloat(e.Weight) {
			return jev.Prediction{}, fmt.Errorf("invalid evidence entry %q", truncUTF8(e.Feature, 64))
		}
	}
...
// classifyGuest pregunta a una réplica del dorado snap (kling jev deploy),
// valida su respuesta contra el registro de despliegue y devuelve la
// predicción en la misma forma que jev.Model.Predict/PredictFull, más las
...
func (g *Gateway) classifyGuest(ctx context.Context, snap string, in jev.Input, explain bool) (jev.Prediction, []string, error) {
	labels, err := g.deployLabels(ctx, snap)
	if err != nil {
		return jev.Prediction{}, nil, err
	}
	...
	if len(b) > maxJEVGuestAnswerBytes {
		return jev.Prediction{}, nil, fmt.Errorf("replica answer larger than %d bytes", maxJEVGuestAnswerBytes)
	}
	if resp.StatusCode != http.StatusOK {
		return jev.Prediction{}, nil, fmt.Errorf("replica answered %d: %s", resp.StatusCode, truncUTF8(string(b), 200))
	}
	var gr jevGuestResponse
	if err := json.Unmarshal(b, &gr); err != nil {
		return jev.Prediction{}, nil, fmt.Errorf("replica answer is not a classify response: %w", err)
	}
	p, err := validateGuestReply(labels, gr)
	if err != nil {
		return jev.Prediction{}, nil, &guestInvalidError{err}
	}
	return p, labels, nil
}
