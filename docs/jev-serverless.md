# JEV serverless: una tarea, un dorado, cero coste ocioso

[JEV](jev.md) vive normalmente **dentro** del proceso del gateway
(`kind: "jev"`, sin daemon, microsegundos). Esta página documenta la
alternativa: cada tarea JEV como su **propia imagen y su propio dorado
...
arranque que solo sabe cargar un `.jev` y contestar `/v1/classify`—.

Por qué existe esto si JEV en proceso ya es rapidísimo: **empaquetado y
aislamiento por tarea**. Cien tareas JEV en proceso comparten el mismo
`kling ai serve`: un modelo corrupto, una fuga de memoria o un cliente que
agota el presupuesto de `-jev-mem` afectan a las demás. Como tarea serverless,
...
24/7" que ya usa VON, aplicado a JEV. El coste es el que mide esta página: una
vuelta de red y, si estaba congelada, un thaw.

```
cliente ──HTTP──> kling ai serve ── /v1/classify ──┬── backend inprocess (por defecto)
                                                     │     jevCache en este proceso, µs, sin daemon
                                                     │
                                                     └── backend microvm
                                                           │
                                                           ...
                                                           │
                                                           kling-jev, dentro de la microVM
                                                           carga UN .jev (+ .jevs opcional) al arrancar
...
| Aislamiento | Ninguno: todas las tareas comparten proceso y el presupuesto `-jev-mem` | Total: cada tarea es su propia microVM, su propia memoria, su propio `mem.file` |
| Empaquetado | Un `.jev` más en el registro | Una imagen propia (el modelo horneado dentro, como el GGUF de un VON) |
...
| Encaja con | La mayoría de las tareas: JEV ya es µs, no hay nada que aislar | Muchas tareas de terceros o de distintos inquilinos, una tarea con un modelo pesado que no debe compartir presupuesto con las demás, o cuando el resto del catálogo (VON, el codificador) ya vive en microVMs y conviene el mismo modelo operativo para todo |
...
umbral, distribución completa y evidencia cuando duda) que `jev.Model` en
proceso.

## `kling jev deploy`

```sh
kling jev deploy commits -model commits.jev -mem 64 -vcpus 1
# Building image "commits" (kling-jev + commits.jev)...
# Making the golden snapshot (1 vCPU, 64 MiB)...
#   booted commits-golden-2yek in 46 ms; waiting for kling-jev...
#   kling-jev ready in 2.063s; warming up...
...
#   {"models": {"commits": {"kind": "jev", "backend": "microvm", "snapshot": "commits"}}}

kling jev ls
# TASK      SNAPSHOT   MEMORY   REPLICAS RUNNING
# commits   commits    44M      0

kling jev rm commits          # borra el dorado y la imagen (-keep-image: solo el dorado)
```
...
dorado ahí, igual que con un modelo VON). El `.jev` (y el `.jevs` de huecos,
opcional) viajan dentro de la petición al constructor `jev`: no hace falta que
el daemon vea el disco de quien manda la orden, así que un daemon remoto por
SSH funciona igual. `-model`/`-slots` se validan (`jev.Load`/`slots.Load`)
antes de escribirse en la imagen: un fichero corrupto no llega a congelarse en
un dorado inservible.

Dentro de la imagen, `kling-jev` (`cmd/kling-jev`) es el invitado entero: un
binario estático (~10 MB, sin cgo, `CGO_ENABLED=0`) que carga el `.jev` al
arrancar y sirve:
...
- `POST /v1/classify` — `{"text", "fields", "explain"}` → `{"label", "prob",
  ...
  rápido sin reservas cuando JEV contesta seguro; si duda o se pide `explain`,
...
{
  "models": {
    "commits": {"kind": "jev", "backend": "microvm", "snapshot": "commits"},
    "urgent":  {"kind": "jev", "path": "urgent.jev"}
  },
  "tasks": {
    "commit-type": {"jev": "commits"},
    "is-urgent":   {"jev": "urgent"}
  }
}
```

`backend` solo se aplica a `kind: "jev"` (von y embed siempre son una
...
prueba es un `.jev` de 3 clases, 2^18 cubos (el tamaño por defecto: ~1,5 MB en
...
| `kling jev deploy` de punta a punta | ~4 s (arranque 46 ms + kling-jev listo 2,06 s + calentamiento + congelar) |
...
planificador, no de kling-jev (que contesta en microsegundos, igual que en
...
medio, JEV es 10-13× más rápido de punta a punta** en esta misma máquina y
...
propio JEV tarda los mismos 1-5 µs que en proceso, ver [jev.md](jev.md), la
...
Esta parte no usa ninguna microVM: es `jevCache`
([jev.md](jev.md), `pkg/aigw/jevcache.go`) por sí sola, con un registro de 0,
10 y 100 modelos (mismo fichero `.jev` de 1,5 MB en memoria copiado a rutas
...
propósito para forzar desalojos (`-jev-mem 8`, 20 modelos usados uno detrás de
otro):

```
kling_ai_jev_models_loaded 5           # el LRU desaloja: solo caben 5 de los 20 pedidos
kling_ai_jev_bytes 7885760             # 7,5 MiB, justo bajo el presupuesto de 8
kling_ai_jev_loads_total 20
kling_ai_jev_evictions_total 15
```
...
µs** — del mismo orden que los 1-5 µs de [jev.md](jev.md), con el resto siendo
JSON y HTTP.
...
Para comparar con JEV puro sin HTTP ni JSON de por medio, en la misma
máquina (`go test ./pkg/jev -bench Predict -cpu 1,4`): 1,8 µs/predicción a un
núcleo, 529 ns/op agregado a 4 núcleos (~1,9 M/s; con los 10 núcleos del M4,
del orden de los ~3,4 M/s de [jev.md](jev.md)). La diferencia entre esto y los
113 000/s de la tabla de arriba es JSON + HTTP + el socket Unix, no JEV: por
eso un cliente que puede permitirse hablar Go directo con `pkg/jev`
