...
| [`intent.md`](intent.md) | Las tareas de intención del gateway de IA (`"intent"`, `pkg/intent`): plantillas → Chispa + Chispa-slots → codificador con puerta; el dominio como esquema JSON o `intent.Domain` en Go | vas a decidir órdenes cortas (intención y huecos) con el gateway |
| [`codificador.md`](codificador.md) | La capa 3 de las tareas de intención: un codificador de frases (multilingual-e5-small, MIT) servido como un VON (kind `embed`), la cabeza `.jenc` en Go, su evaluación honesta, latencia y memoria, y la mejora futura (no aplicada) del ajuste fino con GPU | vas a tocar la capa entre Chispa y el LLM, o a servir embeddings |
...
| [`domotica.md`](domotica.md) · [`domotica-datos.md`](domotica-datos.md) · [`DOMOTICA-EVAL.md`](DOMOTICA-EVAL.md) | El ejemplo de domótica (`kindling-domotica`, un programa aparte en `examples/domotica`): plantillas de la demo → Chispa + Chispa-slots (`.chispas`), los datos libres (MASSIVE, Home Assistant) con su licencia y taxonomía, y su evaluación honesta | vas a tocar la demo de la habitación o la cascada de decisión |
| [`CI-TRIAGE-EVAL.md`](CI-TRIAGE-EVAL.md) | El triaje de fallos de CI de `examples/ci-triage`: Chispa localiza las líneas que explican el fallo en LogChunks (CC BY 4.0) y lo categoriza, VON solo lee el trozo; líneas base, latencia, en proceso frente a microVM, y un dominio que no vio (GitHub Actions) | quieres usar Chispa sobre logs o ver la cascada con un LLM en una tarea nueva |
...
| [`archivo-repos.md`](archivo-repos.md) | Qué se hizo con los repos viejos kindling-mcp y kindling-sandbox al traerlos a `ext/`, y los pasos para archivarlos |
