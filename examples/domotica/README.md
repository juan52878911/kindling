...
navegador ──HTTP/SSE──> kindling-domotica room ──/v1/decide───> kindling-domotica gateway ──> plantillas (en su proceso, µs)
 (la habitación)        (simulador + página)  /v1/generate      │                ├─> Chispa + huecos: microVM, dorado de kling ai chispa deploy (<1 ms despierta)
...
| 2. Chispa | microVM (`kling ai chispa deploy`, backend `microvm`), o el proceso del gateway | intención + huecos de lo que se parece a una orden | si no encaja una plantilla |
...
dorado que hace `kling ai chispa deploy chispa-room -model intent.chispa -slots
...
kling ai model add von-qwen15-dom -model qwen2.5-1.5b-instruct -quant q4_k_m \
...
kling ai chispa deploy chispa-room -model intent.chispa -slots slots.chispas
...
# (1 155 tokens, 15 s en frío) y commit. (`kling ai model add … -prefix system.txt`
...
./kling machine squeeze dom-warm && ./kling save -replace dom-warm x86-qwen15-dom && ./kling rm dom-warm
...
./kling ai model add x86-enc-e5 -model multilingual-e5-small
...
./kling ai chispa deploy chispa-room -model models/intent.chispa -slots models/slots.chispas -warm-text "enciende la luz de la cocina"
