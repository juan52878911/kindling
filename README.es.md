...
**En Mac Apple Silicon** (M3+, **compatibilidad limitada**), Firecracker necesita una VM
Linux arm64 con virtualización anidada; hay un atajo `make deploy-mac HOST=ssh://...` y la
receta reproducible completa —con sus límites honestos— en
...
corre dentro de una VM Linux aarch64 con virtualización anidada — **soportado, con
límites**: el arranque en frío es más lento bajo KVM anidado (~16 s por réplica nueva vs
~3 s en Linux nativo), así que sirve para desarrollo local y evaluación, no para alta
concurrencia. Ver [`docs/mac-arm64.md`](docs/mac-arm64.md).
