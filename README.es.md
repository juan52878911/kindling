<p align="center">
  <img src="docs/kindling-banner.svg" alt="kindling — las ramas secas que prenden primero" width="760">
</p>
...
El nombre es la idea: *kindling* es la leña menuda y seca que prende primero y enciende
el fuego grande. Aquí cada herramienta es kindling — pequeña, fría, sin costar nada
mientras espera, y ardiendo en milisegundos en cuanto se la necesita.
...
Un agente de IA quiere herramientas. La mayoría de servidores MCP open source son
procesos Node o Python pensados para que el cliente los lance y los mantenga vivos
durante toda la conversación. Ese modelo se rompe en cuanto quieres *muchas*
herramientas, de fuentes *no confiadas*, en una máquina *compartida*:

- Veinte procesos siempre encendidos queman RAM y CPU se usen o no.
- Tooling open source arbitrario dirigido por una IA es código no confiado, y el
  namespace de un contenedor es un muro fino que ponerle alrededor.
- Cargar doscientos esquemas de herramientas en el contexto del modelo antes de que
  empiece a trabajar es otro desperdicio distinto.

La respuesta de kindling: cada servidor MCP corre dentro de su propia microVM de
Firecracker, arranca **una vez**, se congela con el servidor ya escuchando, y un
gateway lo restaura bajo demanda. Una herramienta congelada es un fichero en disco
—cero CPU, cero RAM— y despertarla cuesta ~30 ms, imperceptible dentro de una llamada.

## Ventajas de un vistazo

| | |
|---|---|
| **Aislamiento de hipervisor** | Cada herramienta corre tras una frontera KVM, no un namespace de kernel compartido |
| **Coste cero en reposo** | Las máquinas congeladas son ficheros: sin CPU, sin RAM, ~35–82 MB de disco |
| **Despertar en milisegundos** | ~30 ms de restauración; 9 ms por llamada en caliente |
| **12× de densidad** | Los snapshots dorados comparten páginas de memoria: 6,8 MiB por instancia en vez de 82 MiB |
| **Cualquier servidor MCP** | Los de stdio llevan puente; los de Streamable HTTP nativo corren tal cual |
| **Contexto pequeño para el modelo** | Un endpoint agregador con 3 meta-herramientas en vez de N×M esquemas |
| **Nada expuesto** | El daemon nunca escucha en red; el control remoto es solo por SSH |

## kling

El CLI. Si has usado docker, ya lo conoces:
